...
| 2 | [#1](https://github.com/skull-01/OppoKodiBridge-v4/issues/1) `de7545a` | **M9205 (if available):** same play→stop cycle, ideally with a disc whose stop isn't a clean `@UPL STOP`. | Stop is detected within a few seconds (verbose push *or* HTTP cross-check), the `:23` socket is released, and the remote never wedges. No indefinite hang. |
