...
def test_keymap_xml_never_touches_the_volume_keys_with_noop():
    # regression: the leak codes ARE the volume keys (0xf0b9/0xf0b8) -- a noop would disable volume.
    # The keymap must ONLY map volume_up/volume_down to NotifyAll, never emit a <key id> noop.
    xml = volumeir.keymap_xml("volume_up", "volume_down")
    assert "<volume_up>NotifyAll" in xml and "<volume_down>NotifyAll" in xml
    assert "noop" not in xml and "<key id=" not in xml
